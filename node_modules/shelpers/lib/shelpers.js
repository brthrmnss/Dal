...
sh.errors.jumpError = function jumpError(msg, lines, ifFalse , postAdd) {
    if ( ifFalse != null ){
        if (ifFalse != false ) {
            return;
        }
...
function copyFile2(from, to) {
    var contents = sh.readFile(from)
    sh.writeFile(to, contents);
}
...
function getFilesInDirectory(dir, recurse, sortByTime, showNames,
    ...
    if ( filterIncludes != null ) {
        var matches = []
        sh.each(filterIncludes, function processEach(i,fileName){
...
function writeFile(fileName, content, surpressErrors) {
    var fs = require("fs")
 ...
 * @returns {*}
 */
function mergeObjects(mergeThisObject, intoThisObject, onlyIfIntoPropIsNotNull) {
...
each.print = function print (items, prop, save) {
...
sh.async = function async (items,fxAction, fxDone ){
    var async = require('async');
    ...
    async.forEachSeries(items, function processEachTest(item, fxInteration ) {
