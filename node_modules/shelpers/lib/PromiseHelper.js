...
function PromiseHelper() {
    ...
    self.processSettings = function processSettings(userSettings) {
        ...
        self.defaultSettings.showDebug = true
    ...
    /**
     * Special callback. will repeat until fx returns true
     * @param fx
     * @returns {PromiseHelper}
     */
    self.addWaitFor = function addWaitFor(fx) {

        function wrappedFx (token, cb) {
            var intervalID = setInterval(function tryAgain() {
                if (fx() == true) {
        ...
        self.data.methods.count++
        self.lastPromise = self.lastPromise.then(self.w(wrappedFx))
        return self;
    }
    ...
    self.end = function end(fx) {
        ...
        self.addRaw(function () {
            if ( self.token.silentToken != true ) {
                console.log('PromiseHelpers', 'finished.................', self.token.name, self.data.methods.count)
...
//    }
    ...
    self.utils.wait3Secs = function wait3Secs(token, callback) {
        return self.add(function wait3Secs(token, callback) {
